...
cat <<EOF | $KIND create cluster ${KIND_CLUSTER_OPTS} --kubeconfig=./nmr-local-${KIND_CLUSTER_NAME} --config=-
kind: Cluster
...
name: nmr-test-projects-$KIND_CLUSTER_NAME
nodes:
    - role: worker
    - role: worker
    - role: worker
...
cat <<EOF | kubectl --kubeconfig=./nmr-local-${KIND_CLUSTER_NAME} apply -f -
apiVersion: v1
kind: ConfigMap
metadata:
